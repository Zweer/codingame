/// Number Shifting — Local solver (reads grid from stdin, writes moves to stdout)
/// Used by submit.py to solve levels offline.
/// No level code I/O — that's handled by the Python script.

use std::io;
use std::time::Instant;

const TIME_LIMIT_MS: u128 = 60_000; // 60s locally, no CG time pressure
...
fn solve(
    ...
    *nodes += 1;
    if *nodes % 50000 == 0 && start.elapsed().as_millis() > TIME_LIMIT_MS { return false; }
...
fn main() {
    ...
    eprintln!("Grid {}x{}, {} nonzero", w, h, nonzero);
    ...
    if solve(&mut grid, w, h, &mut nz, &mut result, &start, &mut nodes) {
        ...
        eprintln!("Solved in {} nodes, {}ms", nodes, start.elapsed().as_millis());
    } else {
        eprintln!("FAILED after {} nodes, {}ms", nodes, start.elapsed().as_millis());
        std::process::exit(1);
    }
}
