# Music Scores - Code Golf

[:link: Puzzle on CodinGame](https://www.codingame.com/codegolf/expert/music-scores-codesize)

**Level:** codegolf-expert
**Topics:** Encoding, Image processing, Pattern recognition

![](fileservlet?id=855239257985)To rest from his stressful last adventure, _Doctor Who_ decides to attend a music recital performed by Her Majesty the Queen’s Royal Orchestra. Since he doesn’t want to be seen in public, the Doctor uses the TARDIS to channel the music directly from the Opera to his home.  
  
But behold! The infamous [Graske](http://tardis.wikia.com/wiki/Graske) has detected an opportunity to do evil: the sound waves channelled by the TARDIS have created a space-time crack that the alien is using to travel through dimensions. He should soon reach Earth and annihilate the human race. Nothing less could have been expected from this vile creature!  
...
Your mission is to help _Doctor Who_ defeat the Graske. To do so, **you have to implement an interface that can read scanned musical scores directly from paper and translate them into musical notes**.  

![](fileservlet?id=798437117788)

---

The musical scores are fairly simple and only feature half notes (![](fileservlet?id=808178814416)) and quarter notes (![](fileservlet?id=808165036238)). Notes are all represented on a staff and are limited to the following musical notes (either half or quarter): 

![](fileservlet?id=819288278844)
...
You are provided with scanned images of the scores in **black and white** encoded in a simple, yet efficient, form of [RLE](http://en.wikipedia.org/wiki/Run-length%5Fencoding) (Run-Length Encoding): the **DWE** (Doctor Who Encoding) algorithm.  
...
Within the images, the scores and notes have various sizes. To fully understand the challenge at hand, you should check all the images from this [page](https://files.codingame.com/pub/dw/dw2-allimages.html) ![](https://lh3.googleusercontent.com/fZt36ACYlX4Ep1JFINprlSUqWmJHLogD6Jqi4p4R9HNz_lf70UIPTGd8mpNOMu3cGtFzEwpafneckAFUrPvlvDfm2gqdPnT4fsR11aBm6Nv7dzAPocRI-aQOzQ) ![](https://lh3.googleusercontent.com/fZt36ACYlX4Ep1JFINprlSUqWmJHLogD6Jqi4p4R9HNz_lf70UIPTGd8mpNOMu3cGtFzEwpafneckAFUrPvlvDfm2gqdPnT4fsR11aBm6Nv7dzAPocRI-aQOzQ). They correspond to the challenge tests further down.   
