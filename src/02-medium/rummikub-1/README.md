...
[:link: Puzzle on CodinGame](https://www.codingame.com/training/medium/rummikub-1)

**Level:** medium
**Topics:** Sets, Brute-force, Pathfinding, Simulation
...
• COMBINE rowid\_1 rowid\_2: combine two rows.  
...
**nrow\_new lines**: the new rows that are formed after putting goalTile on the table (one row per line). The new rows have either their original rowid, or a new rowid (if it was a new row as a result from splitting an original row). Rows that became unused due to a COMBINE action should be skipped.
