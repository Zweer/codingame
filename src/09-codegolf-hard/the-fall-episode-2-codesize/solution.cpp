...
int g2[]={2,3},g4[]={4,5},g6[]={6,7,8,9},g10[]={10,11,12,13};
RL[0]=1;RG[0][0]=0;RL[1]=1;RG[1][0]=1;
for(int i=0;i<2;i++){RL[2+i]=2;for(int j=0;j<2;j++)RG[2+i][(j+i)%2]=g2[(j+i)%2],RG[2+i][j]=g2[(j+i)%2];}
RL[2]=2;RG[2][0]=2;RG[2][1]=3;RL[3]=2;RG[3][0]=3;RG[3][1]=2;
RL[4]=2;RG[4][0]=4;RG[4][1]=5;RL[5]=2;RG[5][0]=5;RG[5][1]=4;
for(int i=0;i<4;i++){RL[6+i]=4;for(int j=0;j<4;j++)RG[6+i][j]=g6[(j+i)%4];}
for(int i=0;i<4;i++){RL[10+i]=4;for(int j=0;j<4;j++)RG[10+i][j]=g10[(j+i)%4];}
scanf("%d%d",&W,&H);
