...
int main(){
    ...
    for(int i=0;i<h;i++){scanf("%s",g[i]);for(int j=0;j<w;j++)d[i][j]=-1;}
    int sr=0,sc=0;
    ...
    while(qs<qe){int r=qr[qs],c=qc[qs++];
        for(int i=0;i<4;i++){int nr=(r+dr[i]+h)%h,nc=(c+dc[i]+w)%w;
            if(g[nr][nc]!='#'&&d[nr][nc]==-1){d[nr][nc]=d[r][c]+1;qr[qe]=nr;qc[qe++]=nc;}}}
    for(int r=0;r<h;r++){for(int c=0;c<w;c++) putchar(g[r][c]=='#'?'#':d[r][c]==-1?'.':C[d[r][c]]);putchar('\n');}
    return 0;
}
